...
describe("assets routes", () => {
	...
	it("serves the canonical thumbnail generated by asset optimization", async () => {
		mkdirSync(join(assetsDir, "thumbs"), { recursive: true });
		writeFileSync(join(assetsDir, "hero.png"), "original");
		writeFileSync(
			join(assetsDir, "thumbs", "hero.png.thumb.jpg"),
			"cached-thumbnail",
		);

		const res = await fetch(`${baseUrl}/assets/thumb/hero.png`);
		expect(res.status).toBe(200);
		expect(res.headers.get("content-type")).toContain("image/jpeg");
		expect(Buffer.from(await res.arrayBuffer()).toString("utf-8")).toBe(
			"cached-thumbnail",
	...
	it("keeps on-demand thumbnail generation under the canonical name", async () => {
		writeFileSync(
			join(assetsDir, "logo.svg"),
			'<svg xmlns="http://www.w3.org/2000/svg" width="800" height="600"><rect width="800" height="600" fill="red"/></svg>',
		);

		const res = await fetch(`${baseUrl}/assets/thumb/logo.svg`);
		expect(res.status).toBe(200);
		expect(res.headers.get("content-type")).toContain("image/png");
		expect(existsSync(join(assetsDir, "thumbs", "logo.svg.thumb.png"))).toBe(
			true,
		);
	});
	...
	it("accepts an upload body already parsed by the executable JSON middleware", async () => {
		const app = express();
		app.use(express.json({ limit: "5mb" }));
		app.use(
			createAssetsRouter({
				config: { ASSETS_DIR: assetsDir } as Config,
				store,
				bus,
				assets,
			}),
		);
		const mounted = await startTestApp(app);
		try {
			const res = await fetch(`${mounted.baseUrl}/api/upload`, {
				method: "POST",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({
					filename: "parsed.png",
					data: Buffer.from("parsed-body").toString("base64"),
				}),
			});
			expect(res.status).toBe(200);
			expect(readFileSync(join(assetsDir, "parsed.png"), "utf8")).toBe(
				"parsed-body",
			);
		} finally {
			await mounted.close();
