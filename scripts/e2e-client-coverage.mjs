...
const sourceFiles = (await walk(CLIENT_SRC)).filter((file) =>
...
const inScope = productionFiles.filter(
	(file) => /\.(ts|tsx)$/.test(file) && !isOutOfScope(file),
);
...
function buildInventory(map, scope, excluded) {
	...
	for (const file of scope) {
		...
		const summary = libCoverage
			.createFileCoverage(fileCoverage)
			.toSummary();
