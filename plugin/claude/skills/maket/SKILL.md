...
Use `maket_workspace` for session state and user messages, `maket_doc` for document lifecycle, `maket_html` for page composition, `maket_charte` for brand language, `maket_collection` for structured placeholder data, `maket_state` for document-owned living data and revisions, `maket_preview` for visual checks, and `maket_pdf` for export.
